%% Clear all
clear vars;
close all;
clc;
%% Initialization of variables
ripUp  = 12;                                                               % ripUp...passband upper limit
ripLow = 10;                                                               % ripLow...passband lower limit
Rp = ripUp - ripLow;                                                       % Rp...passband attenuation
Rs = 40;                                                                   % Rs...stopband attenuation
Wp = 40000 * 2 * pi;                                                       % Wp...pass-frequency
Ws = 10000 * 2 * pi;                                                       % Ws...stop-frequency
s = tf('s');                                                               % s = tf('s')...allows you to enter the transfer function 
                                                                           %in the next line in symbolic form rather than 
                                                                           %as numerator and denominator vectors.
magnitude = round(10^(ripUp/20));                                          % magnitude...Ainf
Ainf_1 = round(-sqrt(magnitude));                                          % Ainf_1...passband gain 1st section
Ainf_2 = round(-sqrt(magnitude));                                          % Ainf_2...passband gain 2nd section

%%  Designing of Butterworth Type I filter
[N, Wp] = buttord(Wp, Ws, Rp, Rs, 's');                                    % N...order of filter
[B, A] = butter(N, Wp, 'high', 's');                                       % B...numerator polynominal H(s)
                                                                           % A...denominator polynominal H(s)
H = magnitude * tf(B, A);                                                  % H...transfer function
P = pole(H);                                                               % P...poles of TF
H1 = Ainf_1*s^2/( (s - P(3)) * (s - P(4)) );                               % H1...TF of 1st section
H2 = Ainf_2*s^2/( (s - P(1)) * (s - P(2)) );                               % H2...TF of 2nd section
%% H - adaptation of TF to it's prototype
[H_num, H_den] = tfdata(H, 'v');                                           % represents TF's numerator and denominator as two vectors

H_num(1) = H_num(1) / H_den(N+1);                                          % numerator devided by (N+1)-th vector of denominator
H_den(1:1:N+1) = H_den(1:1:N+1) / H_den(N+1);                              % both numerator and denominator are devided by
                                                                           %(N+1)th element of vector of denominator
H = tf(H_num, H_den);                                                      % H...transfer function
%%  H1 - adaptation of TF to it's prototype
[H1_num, H1_den] = tfdata(H1,'v');                                         % represents TF's numerator and denominator as two vectors                                          

H1_num(1) = H1_num(1) / H1_den(3);                                         % numerator devided by (N+1)-th vector of denominator
H1_den(1:1:3) = H1_den(1:1:3) / H1_den(3);                                 % both numerator and denominator are devided by
                                                                           %(N+1)th element of vector of denominator
H1 = tf(H1_num, H1_den)
%%  H2 - adaptation of TF to it's prototype                                                  
[H2_num, H2_den] = tfdata(H2, 'v');                                        % represents TF's numerator and denominator as two vectors

H2_num(1) = H2_num(1) / H2_den(3);                                         % numerator devided by (N+1)-th vector of denominator
H2_den(1:1:3) = H2_den(1:1:3) / H2_den(3);                                 % both numerator and denominator are devided by
                                                                           %(N+1)th element of vector of denominator
H2 = tf(H2_num, H2_den)
%%  Filter coefficients
d1 = H1_den(1);
c1 = H1_den(2);
d2 = H2_den(1);
c2 = H2_den(2);
%%  Electronic components
C2 = 330e-12;                                                              %[C2] = F (chosen)
C3 = 330e-12;                                                              %[C3] = F (chosen)
C5 = 330e-12;                                                              %[C5] = F (chosen)
C6 = 330e-12;                                                              %[C6] = F (chosen)
%C1 = Ainf_1 * C2;
C1 = (330 + 330)*10^-12;                                                   %[C1] = F
%C4 = Ainf_2 * C5;
C4 = (330 + 330)*10^-12;                                                   %[C4] = F
R1 = c1 / (C3 + C2 * (1 - Ainf_1));                                        %[R1] = Ohm
R2 = d1/c1 * (C3 + C2 * (1 - Ainf_2)) / (C2 * C3);                         %[R2] = Ohm
R3 = c2 / (C6 + C5 * (1 - Ainf_2));                                        %[R3] = Ohm
R4 = d2/c2 * (C6 + C5 * (1 - Ainf_2)) / (C5 * C6);                         %[R4] = Ohm
%%  E24 row
C1_e24 = (330 + 330)*10^-12;
C2_e24 = 330e-12;
C3_e24 = 330e-12;
C4_e24 = (330 + 330)*10^-12;
C5_e24 = 330e-12;
C6_e24 = 330e-12;
R1_e24 = 6800 + 240;
R2_e24 = 33000;
R3_e24 = 2700 + 220;
R4_e24 = 75000 + 4700;
%%  transfer functions with the components from the E24
H1_real = tf([-C1_e24*C3_e24*R1_e24*R2_e24 0 0], [C2_e24*C3_e24*R1_e24*R2_e24  R1_e24*(C1_e24+C2_e24+C3_e24)  1]);
H2_real = tf([-C4_e24*C6_e24*R3_e24*R4_e24 0 0], [C5_e24*C6_e24*R3_e24*R4_e24  R3_e24*(C4_e24+C5_e24+C6_e24)  1]);
H_real = H1_real * H2_real;
%%  Provisional Plot.
subplot(2, 1, 1);
h = bodeplot(H);
setoptions(h, 'FreqUnits', 'kHz', 'PhaseVisible', 'off', 'XLim', {[1, 1000]}, 'YLim', {[-40, 20]});
title('Provisional Plot');
hold on;
h1 = bodeplot(H1);
setoptions(h1, 'FreqUnits', 'kHz', 'PhaseVisible', 'off', 'XLim', {[1, 1000]}, 'YLim', {[-40, 20]});
hold on;
h2 = bodeplot(H2);
setoptions(h2, 'FreqUnits', 'kHz', 'PhaseVisible', 'off', 'XLim', {[1, 1000]}, 'YLim', {[-40, 20]});
hold on;
grid on;
legend('H', 'H1', 'H2', "location", "southwest");

%%  Final Plot.
subplot(2, 1, 2);
h_real = bodeplot(H_real);
setoptions(h_real, 'FreqUnits', 'kHz', 'PhaseVisible', 'off', 'XLim', {[1, 1000]}, 'YLim', {[-40, 20]});
title('Final Plot');
hold on;
h1_real = bodeplot(H1_real);
setoptions(h1_real, 'FreqUnits', 'kHz', 'PhaseVisible', 'off', 'XLim', {[1, 1000]}, 'YLim', {[-40, 20]});
hold on;
h2_real = bodeplot(H2_real);
setoptions(h2_real, 'FreqUnits', 'kHz', 'PhaseVisible', 'off', 'XLim', {[1, 1000]}, 'YLim', {[-40, 20]});
hold on;
grid on;
legend('Hreal', 'H1real', 'H2real', "location", "southwest");